...
Этот пример похож на смесь из `HasMany` (в том смысле, что все непредставленные записи будут удалены из пивота) и `BlongsTo` - и все поля, кроме поля `$primaryKey` будут проигнорированы, по причинам изложеным в разделе `belongsTo`. Обратите внимание, что работа с пивотом так же доступна.
